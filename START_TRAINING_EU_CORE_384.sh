#!/bin/bash
# Launch Teacher C (Europa Norte/East) training with 384px + smart crop

set -euo pipefail

REGIONS="EU_NORTH,EU_EAST"
PROJECT_ROOT="$(cd "$(dirname "$0")" && pwd)"
cd "$PROJECT_ROOT"

CONFIG_FILE="config/teacher_eu_core.yaml"
...
DATA_PREFIX="./data/dataset_eu_core"
...
if [ ! -f "$TRAIN_FILE" ] || [ ! -f "$VAL_FILE" ]; then
  echo "⚠️  Splits EU_CORE no encontrados. Generándolos..."
...
if [ ! -f "$TRAIN_FILE" ] || [ ! -f "$VAL_FILE" ]; then
  echo "❌ No se generaron los splits EU_CORE (revisa logs)."
  exit 1
fi

echo "✅ Splits EU_CORE listos"
...
if [ -d "checkpoints/teacher_eu_core" ]; then
  BACKUP_DIR="checkpoints/teacher_eu_core_backup_$(date +%Y%m%d_%H%M%S)"
  echo "⚠️  Respaldo de checkpoints previos → $BACKUP_DIR"
  mv checkpoints/teacher_eu_core "$BACKUP_DIR"
fi

INIT_FROM=$(python3 - <<'PY'
import yaml
with open('config/teacher_eu_core.yaml', 'r') as fh:
...
echo "==============================================================="
echo "  Iniciando entrenamiento Teacher C (Regiones: $REGIONS)"
echo "==============================================================="
echo "Model: vit_base_patch16_384"
echo "Train file: $TRAIN_FILE"
echo "Val file:   $VAL_FILE"
echo "Cache dir:  $CACHE_DIR"
echo "Log file:   training_eu_core_384.log"
echo ""

nohup python3 scripts/train_teacher.py \
  --config "$CONFIG_FILE" \
  > training_eu_core_384.log 2>&1 &

TRAIN_PID=$!

echo "✅ Entrenamiento lanzado (PID $TRAIN_PID)"
echo "Monitoriza con: tail -f training_eu_core_384.log | grep -E 'Epoch|loss|top'"
echo "TensorBoard: tensorboard --logdir ./checkpoints/teacher_eu_core/logs"
echo ""
